...
function u1.Assert(p7) -- Line: 120
    -- upvalues: u1 (copy)
    local v8 = u1.Is(p7);
    assert(v8, "Result was not of type Option");
end;

function u1.Deserialize(p9) -- Line: 125
    -- upvalues: u1 (copy)
    local v10;
...
function u1.Serialize(p11) -- Line: 131
    return {
...
function u1.Match(p12, p13) -- Line: 139
    local Some = p13.Some;
    local None = p13.None;
...
function u1.IsSome(p16) -- Line: 152
    return p16._s;
end;

function u1.IsNone(p17) -- Line: 157
    return not p17._s;
end;

function u1.Expect(p18, p19) -- Line: 162
...
function u1.ExpectNone(p21, p22) -- Line: 168
    local v23 = p21:IsNone();
    assert(v23, p22);
end;

function u1.Unwrap(p24) -- Line: 173
    return p24:Expect("Cannot unwrap option of None type");
end;

function u1.UnwrapOr(p25, p26) -- Line: 178
    if p25:IsSome() then
        return p25:Unwrap();
    end;
...
function u1.UnwrapOrElse(p27, p28) -- Line: 187
    if p27:IsSome() then
        return p27:Unwrap();
    end;
...
function u1.And(p29, p30) -- Line: 196
    -- upvalues: u1 (copy)
    if p29:IsSome() then
        return p30;
    end;
...
function u1.AndThen(p31, p32) -- Line: 205
    -- upvalues: u1 (copy)
...
function u1.Or(p34, p35) -- Line: 216
    if p34:IsSome() then
        return p34;
    end;
...
function u1.OrElse(p36, p37) -- Line: 225
    -- upvalues: u1 (copy)
    if p36:IsSome() then
        return p36;
    end;
...
function u1.XOr(p39, p40) -- Line: 236
    -- upvalues: u1 (copy)
...
function u1.Filter(p42, p43) -- Line: 249
    -- upvalues: u1 (copy)
...
function u1.Contains(p44, p45) -- Line: 258
    local v46 = p44:IsSome() and p44._v == p45;

    return v46;
end;

function u1.__tostring(p47) -- Line: 263
    return not p47:IsSome() and "Option<None>" or "Option<" .. typeof(p47._v) .. ">";
end;

function u1.__eq(p48, p49) -- Line: 272
    -- upvalues: u1 (copy)
    if u1.Is(p49) then
