...
if Packages.IsPlugin then
    ...
    return {
        Theme = {
            GetColor = function(p1, p2) -- Line: 11, Name: GetColor
                -- upvalues: Studio (copy)
                for _, v in pairs(Enum.StudioStyleGuideColor:GetEnumItems()) do
                    if v.Name == p2 then
                        return Studio.Theme:GetColor(v);
                    end;
                end;

                error(tostring(p2) .. "is not a valid member of \"Enum.StudioStyleGuideColor\"");
            end
        },
        ThemeChanged = Studio.ThemeChanged
    };
end;
...
local u3 = {
    ...
    ScriptBracket = Color3.fromHex("#ffd700"),
    AttributeCog = Color3.fromHex("#aaaaaa")
};

return {
    Theme = {
        GetColor = function(p4, p5) -- Line: 154, Name: GetColor
            -- upvalues: u3 (copy)
            return u3[p5];
        end
    },
