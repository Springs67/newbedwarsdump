...
function u3.GetModuleName(p5) -- Line: 149
    return "BaseCamera";
end;

function u3._setUpConfigurations(u6) -- Line: 153
    -- upvalues: LocalPlayer (copy), u2 (copy)
    u6._connections:trackConnection("CHARACTER_ADDED", LocalPlayer.CharacterAdded:Connect(function(p7) -- Line: 154
        -- upvalues: u6 (copy)
    ...
    u6._connections:trackConnection("CAMERA_MODE_CHANGED", LocalPlayer:GetPropertyChangedSignal("CameraMode"):Connect(function() -- Line: 165
        -- upvalues: u6 (copy)
        u6:OnPlayerCameraPropertyChange();
    end));
    u6._connections:trackConnection("CAMERA_MIN_DISTANCE_CHANGED", LocalPlayer:GetPropertyChangedSignal("CameraMinZoomDistance"):Connect(function() -- Line: 168
        -- upvalues: u6 (copy)
        u6:OnPlayerCameraPropertyChange();
    end));
    u6._connections:trackConnection("CAMERA_MAX_DISTANCE_CHANGED", LocalPlayer:GetPropertyChangedSignal("CameraMaxZoomDistance"):Connect(function() -- Line: 171
        -- upvalues: u6 (copy)
        u6:OnPlayerCameraPropertyChange();
    end));
    u6:OnPlayerCameraPropertyChange();
...
function u3.OnCharacterAdded(u8, p9) -- Line: 177
    ...
    if not u2 and UserInputService.TouchEnabled then
        ...
        p9.ChildAdded:Connect(function(p10) -- Line: 190
            -- upvalues: u8 (copy)
            if p10:IsA("Tool") then
                u8.isAToolEquipped = true;
            end;
        end);
        p9.ChildRemoved:Connect(function(p11) -- Line: 195
            -- upvalues: u8 (copy)
...
function u3.GetHumanoidRootPart(p12) -- Line: 204
...
function u3.GetBodyPartToFollow(p14, p15, p16) -- Line: 216
...
function u3.GetSubjectCFrame(p17) -- Line: 228
...
function u3.GetSubjectVelocity(p20) -- Line: 302
    local CurrentCamera = workspace.CurrentCamera;
...
function u3.GetSubjectRotVelocity(p22) -- Line: 331
    local CurrentCamera = workspace.CurrentCamera;
...
function u3.StepZoom(p24) -- Line: 360
    ...
    if math.abs(v25) > 0 then
        ...
        if v25 > 0 then
            ...
            v26 = math.max((currentSubjectDistance + v25) / (1 - v25 * 0.5), 0.5);
        end;

        p24:SetCameraToSubjectDistance(v26 < p24.FIRST_PERSON_DISTANCE_THRESHOLD and 0.5 or v26);
    end;

    return ZoomController.GetZoomRadius();
end;

function u3.GetSubjectPosition(p27) -- Line: 385
...
function u3.OnViewportSizeChanged(p30) -- Line: 462
...
function u3.OnCurrentCameraChanged(u31) -- Line: 470
    -- upvalues: UserInputService (copy)
    if UserInputService.TouchEnabled then
        ...
        if CurrentCamera then
            ...
            u31.viewportSizeChangedConn = CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(function() -- Line: 481
                -- upvalues: u31 (copy)
                u31:OnViewportSizeChanged();
            end);
        end;
    ...
    if CurrentCamera then
        u31.cameraSubjectChangedConn = CurrentCamera:GetPropertyChangedSignal("CameraSubject"):Connect(function() -- Line: 495
            -- upvalues: u31 (copy)
...
function u3.OnPlayerCameraPropertyChange(p32) -- Line: 502
    p32:SetCameraToSubjectDistance(p32.currentSubjectDistance);
end;

function u3.InputTranslationToCameraAngleChange(p33, p34, p35) -- Line: 507
    return p34 * p35;
end;

function u3.GamepadZoomPress(p36) -- Line: 513
...
function u3.Enable(p39, p40) -- Line: 558
...
function u3.OnEnabledChanged(u41) -- Line: 566
    ...
    u41.gamepadZoomPressConnection = CameraInput.gamepadZoomPress:Connect(function() -- Line: 572
        -- upvalues: u41 (copy)
        u41:GamepadZoomPress();
    end);

    if LocalPlayer.CameraMode == Enum.CameraMode.LockFirstPerson then
        u41.currentSubjectDistance = 0.5;
    ...
    u41.cameraChangedConn = workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function() -- Line: 584
        -- upvalues: u41 (copy)
...
function u3.GetEnabled(p42) -- Line: 602
    return p42.enabled;
end;

function u3.Cleanup(p43) -- Line: 606
...
function u3.UpdateMouseBehavior(p44) -- Line: 627
...
function u3.SetCameraType(p49, p50) -- Line: 695
    p49.cameraType = p50;
end;

function u3.GetCameraType(p51) -- Line: 700
    return p51.cameraType;
end;

function u3.SetCameraMovementMode(p52, p53) -- Line: 705
    p52.cameraMovementMode = p53;
end;

function u3.GetCameraMovementMode(p54) -- Line: 709
    return p54.cameraMovementMode;
end;

function u3.SetIsMouseLocked(p55, p56) -- Line: 713
    p55.inMouseLockedMode = p56;
end;

function u3.GetIsMouseLocked(p57) -- Line: 717
    return p57.inMouseLockedMode;
end;

function u3.SetMouseLockOffset(p58, p59) -- Line: 721
    p58.mouseLockOffset = p59;
end;

function u3.GetMouseLockOffset(p60) -- Line: 725
    return p60.mouseLockOffset;
end;

function u3.InFirstPerson(p61) -- Line: 729
    return p61.inFirstPerson;
end;

function u3.EnterFirstPerson(p62) -- Line: 733
    p62.inFirstPerson = true;
    p62:UpdateMouseBehavior();
end;

function u3.LeaveFirstPerson(p63) -- Line: 738
    p63.inFirstPerson = false;
    p63:UpdateMouseBehavior();
end;

function u3.GetCameraToSubjectDistance(p64) -- Line: 744
    return p64.currentSubjectDistance;
end;

function u3.GetMeasuredDistanceToFocus(p65) -- Line: 751
    local CurrentCamera = game.Workspace.CurrentCamera;
...
function u3.CalculateNewLookVectorFromArg(p75, p76, p77) -- Line: 773
    return p75:CalculateNewLookCFrameFromArg(p76, p77).LookVector;
end;

function u3.CalculateNewLookVectorVRFromArg(p78, p79) -- Line: 778
    local unit = ((p78:GetSubjectPosition() - game.Workspace.CurrentCamera.CFrame.p) * Vector3.new(1, 0, 1)).unit;
...
function u3.GetHumanoid(p82) -- Line: 788
    -- upvalues: LocalPlayer (copy)
...
function u3.GetHumanoidPartToFollow(p86, p87, p88) -- Line: 806
...
function u3.OnNewCameraSubject(p89) -- Line: 820
...
function u3.IsInFirstPerson(p90) -- Line: 827
    return p90.inFirstPerson;
end;

function u3.Update(p91, p92) -- Line: 831
    error("BaseCamera:Update() This is a virtual function that should never be getting called.", 2);
end;

function u3.GetCameraHeight(p93) -- Line: 835
