...
return {
    AgniItemTimerApp = v4.new(u3)(function(p6, p7) -- Line: 17
        -- upvalues: Players (copy), u2 (copy), InventoryUtil (copy), ItemType (copy), AgniKitUtil (copy), Workspace (copy), u3 (copy), TooltipContainer (copy), AutoSizedText (copy), ColorUtil (copy), Empty (copy)
        local useState = p7.useState;
        local useEffect = p7.useEffect;
        local v8 = Players.LocalPlayer:GetAttribute("NextClusterReceiveTime");
        ...
        useEffect(function() -- Line: 28
            ...
            local u15 = u2.new();
            u15:GiveTask(Players.LocalPlayer:GetAttributeChangedSignal("NextClusterReceiveTime"):Connect(function() -- Line: 30
                -- upvalues: Players (ref), u10 (ref)
                local v16 = Players.LocalPlayer:GetAttribute("NextClusterReceiveTime");
        ...
        useEffect(function() -- Line: 40
            -- upvalues: InventoryUtil (ref), Players (ref), ItemType (ref), AgniKitUtil (ref), u14 (copy), u9 (copy), Workspace (ref), u12 (copy)
            local u17 = true;

            local function u20() -- Line: 43
                -- upvalues: u17 (ref), InventoryUtil (ref), Players (ref), ItemType (ref), AgniKitUtil (ref), u14 (ref), u9 (ref), Workspace (ref), u12 (ref), u20 (ref)
                task.delay(1, function() -- Line: 44
                    ...
                    if InventoryUtil.getAmount(Players.LocalPlayer, ItemType.CLUSTER_BOMB) >= AgniKitUtil.MAX_CLUSTER_NADE_SIZE then
                        u14(true);
                    else
                        u14(false);
                    end;
                    ...
                    local v19 = 1 - math.round(v18) / AgniKitUtil.CLUSTER_INTERVAL;
        ...
        local v22 = { u3.createElement("UICorner", {
                CornerRadius = UDim.new(2)
                    ...
                    TextSize = 14,
                    RichText = true,
                    Text = "Max Cluster Bombs: " .. tostring(AgniKitUtil.MAX_CLUSTER_NADE_SIZE),
                    Font = Enum.Font.SourceSans,
                    TextColor3 = Color3.fromRGB(191, 191, 191),
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Limits = Vector2.new(300, 60)
        ...
        return u3.createFragment({
            ClusterBombTimer = u3.createElement("Frame", v21, v22)
        });
    end)
};