...
local PlacementScoreCard = RuntimeLib.import(script, script.Parent, "placement-score-card").PlacementScoreCard;
local v37 = v3.new(u2)(function(u5, p6) -- Line: 16
    -- upvalues: u2 (copy), ClientSyncEvents (copy), MatchState (copy), Players (copy), GamePlayerUtil (copy), PlacementScoreCard (copy), DeviceUtil (copy), ColorUtil (copy), Empty (copy)
    local _ = p6.useState;
    ...
    local function v26(p15, p16) -- Line: 74
        -- upvalues: u5 (copy), Players (ref), GamePlayerUtil (ref), u2 (ref), PlacementScoreCard (ref)
        ...
        return u2.createElement(PlacementScoreCard, {
            Name = name,
            TeamColor = v18.color,
            Score = score,
            Placement = p16 + 1,
    ...
    if v34 then
        ...
        v35.ShowMaxScore = u5.showMaxScore;
        v34 = u2.createElement(PlacementScoreCard, v35);
    end;
    ...
    return u2.createFragment({
        PlacementScores = u2.createElement(Empty, v30, v31)
    });
end);

return {
    PlacementScores = v4.connect(function(p38, p39) -- Line: 199
        local v40 = {};
        ...
        table.sort(teamScores, function(p41, p42) -- Line: 206
            return p41.teamId < p42.teamId;
        end);
        table.sort(teamScores, function(p43, p44) -- Line: 210
            if not p43 then
                return false;
            end;
