...
local ItemCatalogPopup = RuntimeLib.import(script, script.Parent, "ui", "item-catalog-popup").ItemCatalogPopup;
...
function u1.constructor(p3) -- Line: 26
    ...
    p3.Name = "ItemCatalogController";
end;

function u1.KnitStart(p4) -- Line: 30
    -- upvalues: KnitController (copy), default (copy), PlaceUtil (copy), SoundManager (copy), GameSound (copy), Flamework (copy), Players (copy), ItemCatalogPopup (copy)
    KnitController.KnitStart(p4);
    default.Client:Get("ItemCatalogDiscovery"):Connect(function(p5, p6) -- Line: 32
        -- upvalues: PlaceUtil (ref), SoundManager (ref), GameSound (ref), Flamework (ref), Players (ref), ItemCatalogPopup (ref)
