...
return function(p3) -- Line: 11
    -- upvalues: u2 (copy), GameReducer (copy), u1 (copy), StoreProvider (copy), MapPublishForm (copy), BedwarsImageId (copy), MapBrowserTags (copy), QueueType (copy)
    local v4 = u2.Store.new(u2.combineReducers({
        Game = GameReducer
    }), {});
    local u5 = u1.mount(u1.createElement(StoreProvider, {
        store = v4
    }, { u1.createElement(MapPublishForm, {
            ...
            MapSaveDisplayInfo = {
                ...
                published = true,
                copylock = true,
                image = BedwarsImageId.DODO_BIRD_RENDER,
                tags = { MapBrowserTags.BEDWARS },
