...
function u3.constructor(p5) -- Line: 27
    ...
    p5.Name = "SpringPunchGadgetHandController";
    p5.maid = u1.new();
    p5.lastAttackTime = 0;
    p5.enableTime = 0;
end;
...
function u3.onEnable(u7, p8) -- Line: 37
    ...
    local v12 = Flamework.resolveDependency("@easy-games/game-core:client/controllers/keybind/action-binder-controller@ActionBinderController"):bindAction({
        ...
        boundFunction = function(p9, p10, p11) -- Line: 43, Name: boundFunction
            -- upvalues: u7 (copy)
        ...
        mobile = {
            ...
            mobileBoundFunction = function() -- Line: 50, Name: mobileBoundFunction
                -- upvalues: u7 (copy)
                u7:attack();
            end
        }
...
function u3.onDisable(p13) -- Line: 57
    p13.maid:DoCleaning();
end;

function u3.isRelevantItem(p14, p15) -- Line: 60
    -- upvalues: ItemType (copy)
    return p15.itemType == ItemType.LOBBY_SPRING_PUNCH;
end;

function u3.attack(p16) -- Line: 63
    -- upvalues: Workspace (copy), Players (copy), KnitClient (copy), default (copy)
    if Workspace:GetServerTimeNow() - p16.enableTime < 0.1 then
        return nil;
    end;

    if Workspace:GetServerTimeNow() - p16.lastAttackTime < 0.25 then
        return nil;
    end;
    ...
    task.spawn(function() -- Line: 74
        ...
        KnitClient.Controllers.SpringPunchGadgetController:animatePunch(Players.LocalPlayer, nil);
    end);
    default.Client:Get("LobbySpringPunchFromClient"):SendToServer({
