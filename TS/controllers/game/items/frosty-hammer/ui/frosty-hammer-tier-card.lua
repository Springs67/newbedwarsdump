...
local getFrostyHammerUpgradeMeta = RuntimeLib.import(script, game:GetService("ReplicatedStorage"), "TS", "games", "bedwars", "items", "frosty-hammer", "frosty-hammer-upgrades").getFrostyHammerUpgradeMeta;
local InventoryUtil = RuntimeLib.import(script, game:GetService("ReplicatedStorage"), "TS", "inventory", "inventory-util").InventoryUtil;
local default = RuntimeLib.import(script, game:GetService("ReplicatedStorage"), "TS", "remotes").default;

return {
    FrostyHammerTierCard = v3.new(u2)(function(u4, p5) -- Line: 13
        -- upvalues: getFrostyHammerUpgradeMeta (copy), InventoryUtil (copy), Players (copy), u2 (copy), ColorUtil (copy), LineFrame (copy), LineButton (copy), default (copy)
        local _ = p5.useState;
        local v6 = u4.UpgradeMap[u4.Upgrade];
        local v7 = v6 == nil and 0 or v6;
        local v8 = getFrostyHammerUpgradeMeta(u4.Upgrade);
        local v9;

        if v7 + 1 <= #v8.tiers then
            v9 = v8.tiers[v7 + 1];
        else
            v9 = nil;
        end;
        ...
        local v14 = u4.Locked and v7 >= 2 and true or false;
        ...
        local v28 = {
            ...
            Position = UDim2.fromScale(0.5, 0.5),
            AnchorPoint = Vector2.new(0.5, 0.5),
            Color3 = ColorUtil.hexColor(6937087),
            Text = not v9 and "MAXED" or (v14 and "LOCKED" or tostring(v9.price) .. " FROST CRYSTAL" .. (v9.price == 1 and "" or "S")),

            OnClick = function() -- Line: 135, Name: OnClick
                -- upvalues: default (ref), u4 (copy)
                default.Client:Get("UpgradeFrostyHammer"):CallServerAsync(u4.Upgrade):andThen(function(p27) -- Line: 137
                    -- upvalues: u4 (ref)
                    if p27 then
                        u4.SetUpgrades(p27);
                    end;
