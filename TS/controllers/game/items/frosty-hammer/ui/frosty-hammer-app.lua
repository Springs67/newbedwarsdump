...
local v4 = u2.Component:extend("FrostyHammerUpgradeAppBase");

function v4.init(p5) -- Line: 16
end;

function v4.render(p6) -- Line: 18
    -- upvalues: InventoryUtil (copy), Players (copy), ItemType (copy), u2 (copy), DarkBackground (copy), SlideIn (copy), DeviceUtil (copy), FrostyHammerComponent (copy)
    local v7 = InventoryUtil.getToolFromInventory(Players.LocalPlayer, ItemType.FROSTY_HAMMER);
    ...
    }), u2.createElement(FrostyHammerComponent, {
        tool = v7
    }) });
...
return {
    FrostyHammerUpgradeApp = v3.connect(function(p15, p16) -- Line: 51
        return p16;
    end)(v4)
};