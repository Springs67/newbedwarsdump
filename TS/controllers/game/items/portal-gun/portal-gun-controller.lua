...
function u2.KnitStart(p5) -- Line: 32
    ...
    ClientSyncEvents.ProjectileLaunched:connect(function(p6) -- Line: 34
        ...
        if p6:isLocalShooter() and p6.projectileType == "portal_projectile" then
            Flamework.resolveDependency("@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController"):setOnCooldown(CooldownId.PORTAL_GUN, getItemMeta(ItemType.PORTAL_GUN).projectileSource.fireDelaySec);
        end;
    end);
    ClientSyncEvents.ProjectileRender:connect(function(p7) -- Line: 39
        if p7.projectile.Name == "portal_projectile" then
            p7.cframe = p7.cframe * CFrame.Angles(0, p7.totalTime * 2 * 3.141592653589793, 0);
        end;
    end);
    ClientSyncEvents.ProjectileLaunched:connect(function(p8) -- Line: 46
        -- upvalues: SoundManager (ref), GameSound (ref)
        if p8.projectileType == "portal_projectile" then
    ...
    KnitClient.Controllers.PreloadController:preloadForItemType(ItemType.PORTAL_GUN, {
...
function u2.isRelevantItem(p10, p11) -- Line: 62
    ...
    return p11.itemType == ItemType.PORTAL_GUN;
end;
...
function u2.onEnable(p12, p13, p14) -- Line: 65
    ...
    p12:setupYield(function() -- Line: 66
        ...
        local u15 = u1.new();
        u15:GiveTask(Flamework.resolveDependency("@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController"):createCooldownBar(CooldownId.PORTAL_GUN));
