...
local EnchantApp = RuntimeLib.import(script, script.Parent, "enchant-app").EnchantApp;

return function(p3) -- Line: 9
    -- upvalues: u2 (copy), GameReducer (copy), u1 (copy), StoreProvider (copy), EnchantApp (copy), ItemType (copy)
    local v4 = u2.Store.new(u2.combineReducers({
        Game = GameReducer
    }), {});
    local u5 = u1.mount(u1.createElement(StoreProvider, {
        store = v4
    }, { u1.createElement(EnchantApp, {
            AppId = "etable",
            EnchantTableType = ItemType.ENCHANT_TABLE_GLITCHED
        }) }), p3);
