...
local SocialApp = RuntimeLib.import(script, script.Parent, "social-app").SocialApp;

return function(p3) -- Line: 8
    -- upvalues: u2 (copy), GameReducer (copy), u1 (copy), StoreProvider (copy), SocialApp (copy)
    local v4 = u2.Store.new(u2.combineReducers({
        Game = GameReducer
    }));
    local u5 = u1.mount(u1.createElement(StoreProvider, {
        store = v4
    }, { u1.createElement(SocialApp, {
            AppId = "SocialApp"
        }) }), p3);
