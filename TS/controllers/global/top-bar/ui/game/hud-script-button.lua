...
local v14 = v3.new(u2)(function(u6, p7) -- Line: 18
    ...
    useEffect(function() -- Line: 22
        ...
        if KnitClient.Controllers.PermissionController:hasAnyPermissions({ "anticheat_mod", "admin", "artist", "director" }) then
            u9(true);

            return nil;
        end;

        local u11 = Players.LocalPlayer:GetAttributeChangedSignal(CustomMatchRoleAttribute):Connect(function() -- Line: 34
            -- upvalues: Players (ref), CustomMatchRoleAttribute (ref), u9 (ref), CustomMatchRole (ref)
            local v10 = Players.LocalPlayer:GetAttribute(CustomMatchRoleAttribute);
            u9(v10 == CustomMatchRole.COHOST and true or v10 == CustomMatchRole.HOST);
        end);
...
return {
    HudScriptButton = v4.connect(function(p15, p16) -- Line: 69
        local v17 = {};
        ...
        local customMatch = p15.Game.customMatch;

        if customMatch ~= nil then
            customMatch = customMatch.hostUserId;
        end;

        v17.CustomHostUserId = customMatch;
