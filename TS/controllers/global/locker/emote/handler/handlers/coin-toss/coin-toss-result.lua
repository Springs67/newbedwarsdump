...
return {
    CoinTossResult = v3.new(u2)(function(p4, p5) -- Line: 9
        ...
        useEffect(function() -- Line: 13
            ...
            task.delay(1.5, function() -- Line: 34
                -- upvalues: KnitClient (ref), u9 (copy), u10 (copy), u8 (copy), u2 (ref)
                local guiTree = KnitClient.Controllers.CoinTossHandlerController.guiTree;

                if guiTree then
                    u9:Play();
                    u10:Play();
                    u8:GiveTask(u10.Completed:Connect(function() -- Line: 39
                        -- upvalues: u2 (ref), guiTree (copy)
                        u2.unmount(guiTree);
                    end));
                end;
            end);
        ...
        return u2.createFragment({
            CoinTossEmoteGui = u2.createElement("BillboardGui", {
                Size = UDim2.fromScale(0, 0),
                StudsOffsetWorldSpace = Vector3.new(0, 2.5, 0),
                Adornee = p4.Character.Head,
                [u2.Ref] = u6
            }, {
                CoinTossEmoteResult = u2.createElement("Frame", {
                    BorderSizePixel = 0,
