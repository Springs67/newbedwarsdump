...
return {
    SummonStoneItemTimerApp = v4.new(u3)(function(p6, p7) -- Line: 18
        -- upvalues: Players (copy), u2 (copy), Workspace (copy), ClientSyncEvents (copy), SpiritSummonerBalance (copy), u3 (copy), TooltipContainer (copy), AutoSizedText (copy), ColorUtil (copy), getItemMeta (copy), ItemType (copy), Empty (copy)
        local useState = p7.useState;
        local useEffect = p7.useEffect;
        local v8 = Players.LocalPlayer:GetAttribute("NextSummonStoneReceiveTime");
        local v9, u10 = useState(0);
        local u11, u12 = useState(v8 == nil and 0 or v8);
        local v13, u14 = useState(false);
        useEffect(function() -- Line: 29
            -- upvalues: u2 (ref), Players (ref), u12 (copy)
            local u15 = u2.new();
            u15:GiveTask(Players.LocalPlayer:GetAttributeChangedSignal("NextSummonStoneReceiveTime"):Connect(function() -- Line: 31
                -- upvalues: Players (ref), u12 (ref)
                local v16 = Players.LocalPlayer:GetAttribute("NextSummonStoneReceiveTime");
        ...
        useEffect(function() -- Line: 41
            -- upvalues: Players (ref), u14 (copy), u11 (copy), Workspace (ref), ClientSyncEvents (ref), SpiritSummonerBalance (ref), u10 (copy)
            local u17 = true;

            local function u20() -- Line: 44
                -- upvalues: u17 (ref), Players (ref), u14 (ref), u11 (ref), Workspace (ref), ClientSyncEvents (ref), SpiritSummonerBalance (ref), u10 (ref), u20 (ref)
                task.delay(1, function() -- Line: 45
                    ...
                    if Players.LocalPlayer:GetAttribute("DisableSummonStonePurchase") == true then
                        u14(true);
                    else
                        u14(false);
                    end;

                    local v18 = u11 - Workspace:GetServerTimeNow();
                    local v19 = 1 - math.round(v18) / ClientSyncEvents.ResourceSpawnDelayModifierCheck:fire(SpiritSummonerBalance.PASSIVE_RECEIVE_TIME).spawnDelay;
                    u10((math.clamp(v19, 0, 1)));
                    u20();
                end);
            end;
        ...
        local v24 = { u3.createElement("UIListLayout", {
                ...
                BackgroundColor3 = Color3.fromRGB(0, 0, 0)
                    ...
                    Size = UDim2.fromScale(1, v13 and 1 or math.clamp(v9, 0, 1))
                ...
                Image = getItemMeta(ItemType.SUMMON_STONE).image
            }) };
        local v25 = #v24;
        ...
        return u3.createFragment({
            SummonStoneTimer = u3.createElement("Frame", v21, v22)
        });
    end)
};