...
local LassoWarsTutorialApp = RuntimeLib.import(script, script.Parent, "lasso-wars-tutorial-app").LassoWarsTutorialApp;

return function(p3) -- Line: 9
    -- upvalues: u2 (copy), GameReducer (copy), u1 (copy), StoreProvider (copy), LassoWarsTutorialApp (copy), BedwarsAppIds (copy)
    local v4 = u2.Store.new(u2.combineReducers({
        Game = GameReducer
    }), {});
    local u5 = u1.mount(u1.createElement(StoreProvider, {
        store = v4
    }, { u1.createElement(LassoWarsTutorialApp, {
