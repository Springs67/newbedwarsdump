...
function u1.new() -- Line: 31
    ...
    u2:GetBindableToggleEvent():Connect(function() -- Line: 40
        -- upvalues: u2 (copy)
        script:SetAttribute("MouseLocked", u2:GetIsMouseLocked());
    end);
    ...
    if BoundKeys then
        BoundKeys.Changed:Connect(function(p3) -- Line: 60
            -- upvalues: u2 (copy)
    ...
    GameSettings.Changed:Connect(function(p4) -- Line: 67
        -- upvalues: u2 (copy)
        if p4 == "ControlMode" or p4 == "ComputerMovementMode" then
            u2:UpdateMouseLockAvailability();
        end;
    end);
    Players.LocalPlayer:GetPropertyChangedSignal("DevEnableMouseLock"):Connect(function() -- Line: 74
        -- upvalues: u2 (copy)
        u2:UpdateMouseLockAvailability();
    end);
    Players.LocalPlayer:GetPropertyChangedSignal("DevComputerMovementMode"):Connect(function() -- Line: 79
        -- upvalues: u2 (copy)
        u2:UpdateMouseLockAvailability();
    end);
    u2:UpdateMouseLockAvailability();
...
function u1.GetIsMouseLocked(p5) -- Line: 88
    return p5.isMouseLocked;
end;

function u1.GetBindableToggleEvent(p6) -- Line: 92
    return p6.mouseLockToggledEvent.Event;
end;

function u1.GetMouseLockOffset(p7) -- Line: 96
    return Vector3.new(1.75, 0, 0);
end;

function u1.UpdateMouseLockAvailability(p8) -- Line: 100
...
function u1.OnBoundKeysObjectChanged(p10, p11) -- Line: 112
    p10.boundKeys = {};
...
function u1.OnMouseLockToggled(p12) -- Line: 127
...
function u1.DoMouseLockSwitch(p13, p14, p15, p16) -- Line: 152
...
function u1.BindContextActions(u17) -- Line: 160
    -- upvalues: ContextActionService (copy), Value (copy)
    ContextActionService:BindActionAtPriority("MouseLockSwitchAction", function(p18, p19, p20) -- Line: 161
        -- upvalues: u17 (copy)
        return u17:DoMouseLockSwitch(p18, p19, p20);
    end, false, Value, unpack(u17.boundKeys));
end;

function u1.UnbindContextActions(p21) -- Line: 166
    -- upvalues: ContextActionService (copy)
    ContextActionService:UnbindAction("MouseLockSwitchAction");
end;

function u1.IsMouseLocked(p22) -- Line: 170
    return p22.enabled and p22.isMouseLocked;
end;

function u1.EnableMouseLock(p23, p24) -- Line: 174
    -- upvalues: CameraUtils (copy)
